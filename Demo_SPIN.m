...
for nSig = [10 20]
    for SpikyRatio = [0.15 0.3]
        ...
        for i = 1:im_num
            ...
            fprintf('The initial value of PSNR = %2.2f  SSIM=%2.4f\n', csnr( noiseI*255, I*255 , 0, 0 ), cal_ssim(noiseI*255, I*255 , 0, 0 ));
            %% AMF
            ...
            imname = sprintf('./GauSPIN/EPLL_AMF_GauSPIN_%d_%2.2f_%s',nSig,SpikyRatio,im_dir(i).name);
            imwrite(cleanI,imname);
            fprintf('%s : PSNR = %2.4f, SSIM = %2.4f \n',im_dir(i).name,csnr( cleanI*255, I*255, 0, 0 ),cal_ssim( cleanI*255, I*255, 0, 0 ));
        end
        mPSNR = mean(PSNR);
        mSSIM = mean(SSIM);
        name = sprintf('EPLL_AMF_GauSPIN_%d_%2.2f.mat',nSig,SpikyRatio);
